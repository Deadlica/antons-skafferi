...
//check grejer
    ...
    private double calculateAverage(){
        if (holdAllReviews == null || holdAllReviews.length == 0) return 0;
        double sum = 0;
        for(Comment c : holdAllReviews){
            sum = sum + c.ranking;
        }
        return sum / holdAllReviews.length;
    }
    private HttpResponse<String> addReview() throws URISyntaxException, IOException, InterruptedException {
        ...
        HttpRequest request = HttpRequest.newBuilder(new URI(API.link + "review"))
